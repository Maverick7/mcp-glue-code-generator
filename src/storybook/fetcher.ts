...
import type { StorybookIndex, DiscoveredDesignSystem, DesignSystemComponent } from './types.js';

export async function fetchStorybookData(storybookUrl: string): Promise<DiscoveredDesignSystem> {
  ...
  if (!indexData) {
    throw new Error(
      `Cannot fetch Storybook data from "${storybookUrl}".\n` +
...
function parseStorybookIndex(index: StorybookIndex, baseUrl: string): DiscoveredDesignSystem {
  const entries = index.entries ?? index.stories ?? {};
  const componentMap = new Map<string, DesignSystemComponent>();
  let totalStories = 0;
