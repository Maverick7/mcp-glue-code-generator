...
if (args.includes('--list-orgs')) {
  ...
  for (const ds of KNOWN_DESIGN_SYSTEMS) {
    ...
    if (ds.npmPackage) console.log(`  Package   : ${ds.npmPackage}`);
  }
...
async function runCli() {
  ...
  if (orgNameOverride) ds.organizationName = orgNameOverride;
  else if (orgArg) {
    const known = findDesignSystemByOrg(orgArg);
    if (known) ds.organizationName = known.organization;
  }
