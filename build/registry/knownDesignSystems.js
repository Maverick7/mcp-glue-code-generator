 ...
 */
export const KNOWN_DESIGN_SYSTEMS = [
    {
        name: 'Atlaskit',
...
export function searchDesignSystems(query) {
    const q = query.toLowerCase().trim();
    if (!q)
        return KNOWN_DESIGN_SYSTEMS;
    return KNOWN_DESIGN_SYSTEMS.filter(ds => ds.name.toLowerCase().includes(q) ||
...
export function findDesignSystemByOrg(orgName) {
    const q = orgName.toLowerCase().trim();
    return KNOWN_DESIGN_SYSTEMS.find(ds => ds.organization.toLowerCase() === q ||
        ds.organization.toLowerCase().includes(q) ||
        ds.name.toLowerCase() === q ||
        ds.name.toLowerCase().includes(q));
}
//# sourceMappingURL=knownDesignSystems.js.map