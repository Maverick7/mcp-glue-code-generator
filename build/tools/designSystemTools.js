...
import { searchDesignSystems, findDesignSystemByOrg, KNOWN_DESIGN_SYSTEMS, } from '../registry/knownDesignSystems.js';
import { generateMcpServer } from '../generators/mcpServerGenerator.js';
// Session-scoped cache so repeated calls don't re-fetch
const cache = new Map();
async function getDesignSystem(url) {
...
export function registerDesignSystemTools(server) {
    // ─────────────────────────────────────────────────────────────────────────
    // Tool 1: discover_design_system
    // ─────────────────────────────────────────────────────────────────────────
    server.tool('discover_design_system', 'Discover and explore any organization\'s design system. Accepts a Storybook URL or an organization name (e.g. "Shopify", "IBM", "GitHub"). Lists all components, categories, and available stories.', {
        url_or_org_name: z.string().describe('A Storybook URL (e.g. "https://react.carbondesignsystem.com") or an org/system name (e.g. "IBM", "Shopify", "GitHub", "Atlassian", "Adobe", "Microsoft")'),
        ...
        try {
            ...
            const knownMatch = KNOWN_DESIGN_SYSTEMS.find(k => k.storybookUrl.includes(new URL(ds.url).hostname));
            return {
                content: [{
                        type: 'text',
                        text: formatDiscoveryResult(ds, {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool('list_design_system_components', 'List and search components from any design system Storybook. Filter by category, search by name, or browse all.', {
        ...
        try {
            ...
            if (category) {
                components = components.filter(c => c.category.toLowerCase().includes(category.toLowerCase()));
            }
            if (search) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool('preview_design_system_component', 'Render a live preview of any design system component story directly in the chat using the Storybook iframe embed.', {
        storybook_url: z.string().describe('The Storybook base URL'),
        ...
        try {
            ...
            return {
                content: [
                    ...
                    { type: 'resource', resource: uiPreview.resource },
                ],
            };
        }
        catch (e) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    server.tool('generate_design_system_mcp', 'Generate a complete, standalone MCP server customized for any organization\'s design system. The generated server can be added to Claude Desktop, VS Code Copilot, or any MCP-compatible chatbot or agent.', {
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function formatDiscoveryResult(ds, extra) {
...
//# sourceMappingURL=designSystemTools.js.map